...
int main(){
    ...
    for(int i=0; i<n; i++)
        if(q[i]=='A')pos.push_back({i, 'A'}), a++;
        else if(q[i]=='B')pos.push_back({i, 'B'}), b++;
    ...
    for(size_t i=0; i<pos.size();){
        ...
        if(pos[i].second=='A'){
            if(last_a_pos==-1)
                ...
                a += pos[i].first - last_a_pos-1;
            last_a_pos=pos[i].first;
            i++;
        }
        else{
            if(i<pos.size()-1){
                if(pos[i+1].second=='B'){
                    b+= pos[i+1].first-pos[i].first-1;
                    i++;
                }
                else{
                    b+= (pos[i+1].first-pos[i].first-1)/2;
                    a+= (pos[i+1].first-pos[i].first-1)/2;
                    last_a_pos=pos[i+1].first;
                    i+=2;
                }
            }else{
                b+= n-pos[i].first-1;
    ...
    // write(a, " "), write(b);
    if(a==b)write("Coalition government");
    else if(a>b)write("A");
    else write("B");
    return 0;
}
